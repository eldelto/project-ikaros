...
int mpu6050_init_dmp(i2c_inst_t* i2c) {
  ...
  unsigned short i;
  size_t writeSize = 0;
  for (i = 0; i < DMP_CODE_SIZE; i += writeSize) {
    writeSize = min(CHUNK_SIZE, DMP_CODE_SIZE - i);

    int result = mpu6050_write_mem(i2c, i, writeSize, (uint8_t*)&dmp_firmware[i]);
    if (result < 0) {
      return result;
    }
  }
